...
//void runAnalysisTrainJoey_MC(const Char_t* infile="/alice/data/current.txt", const Char_t* runmode = "local", const Char_t* inputType="reducedEvent", Bool_t hasMC = kFALSE, // Bool_t isInjected = kFALSE,
void runAnalysisTrainJoey_MC(const Char_t* infile="/alice/sim/current.txt", const Char_t* runmode = "local", const Char_t* inputType="reducedEvent", Bool_t hasMC = kTRUE, // Bool_t isInjected = kFALSE,
                                     Bool_t writeTree = kFALSE, TString tasks="dst", TString prod = "LHC10h", Int_t nEntries=-1, Int_t firstEntry=0,
                                     TString pathForMacros="$ALICE_PHYSICS/PWGDQ/reducedTree/macros") {
   //
   // infile: list of input files if mode is local, or list of runs for job submission if mode is grid
   //
   
   // Setup analysis parameters
   TString macroPath = "/home/joey/alice/AliPhysics/PWGDQ/reducedTree/macros/AddTask_joey_FilterTrees.C";
  ...
  trainadd << (!runmodestr.CompareTo("grid") ? kTRUE : kFALSE) << ", "; // isAliRoot
  trainadd << runMode << ", "; // runMode
  trainadd << (hasMC ? "kTRUE" : "kFALSE") << ", "; // isMC
  //trainadd << (isInjected ? "kTRUE" : "kFALSE") << ", "; // isInjected
  trainadd << "\"" << prod << "\")"; // prod
  ...
  // TODO
  // ROOT5 version
  cout << "The ROOT5 version is not implemented yet! Please use ROOT6 or implement your Task manually!" << endl;
  //gROOT->LoadMacro("$ALICE_PHYSICS/PWGDQ/reducedTree/macros/AddTask_joey_FilterTrees.C");
  //AddTask_TrainTreeAnalysis((!runmodestr.CompareTo("grid") ? kTRUE : kFALSE), prod, writeTree, tasks, pathForMacros);
#endif
